  ...
  class AF_themes_info
  {
    ...
    public function newsium_register_info_page()
    {
      ...
      add_menu_page(
        ...
        'newsium', // Menu slug.
        array($this, 'newsium_render_page'), // Action.
        $afthemes_icon,
        30
      );
      ...
      add_submenu_page(
        'newsium', // Parent slug.
        __('Dashboard', 'newsium'), // Page title.
        __('Dashboard', 'newsium'), // Menu title.
      ...
      add_submenu_page(
        'newsium', // Parent slug.
        __('Upgrade to Pro', 'newsium'), // Page title.
        '<span id="aft-upgrade-menu-item">' . __('Upgrade Now', 'newsium') . '</span>', // Menu title.
        'manage_options', // Capability.
        esc_url('https://afthemes.com/products/newsium-pro/') // Menu slug.
    ...
    public function newsium_render_templates_kits()
    {
      ...
      if (($newsium_elespare_installed && $newsium_elespare_active) && $plugin_update == 'false' && is_plugin_active($newsium_elementor_installed)) {
        ...
        $class = (!empty($newsium_elespare_verison) && $newsium_elespare_active && $newsium_elespare_verison < $newsium_elespare_old_version)
          ...
          : 'false';
        echo '<div id="templatespare-plugin-install-activate" data-class="' . esc_attr($class) . '" current-theme="elespare" install="' . esc_attr(json_encode($install)) . '" activate="' . esc_attr(json_encode($activate)) . '" data-plugin-page="aft-template-kits" message="' . esc_attr($message) . '" isPro="' . esc_attr($newsium_elespare_status) . '"></div>';
      }
    }
